...
async def pack_kangish(_):
    ...
    _packname = cmdtext or f"Jarvis Kang Pack By {_.sender_id}"
    typee = None
    if not local:
    ...
    try:
        short_name = "jar_" + _packname.replace(" ", "_") + str(_.id)
        _r_e_s = await asst(
...
async def hehe(args):
    ...
    if message.photo:
        photo = io.BytesIO()
        photo = await jarvis_bot.download_media(message.photo, photo)
    elif message.file and "image" in message.file.mime_type.split("/"):
        photo = io.BytesIO()
        await jarvis_bot.download_file(message.media.document, photo)
        if (
        ...
        if (message.file.duration or 0) <= 10:
            ...
            heh, lol = y.read()
            cv2.imwrite("jar.webp", lol)
            photo = "jar.webp"
    elif message.file and "tgsticker" in message.file.mime_type:
        await jarvis_bot.download_file(
    ...
    if photo:
        ...
        if not emoji:
            emoji = "🏵"
        if len(splat) == 3:
            pack = splat[2]  # User sent jarvis_both
            emoji = splat[1]
        elif len(splat) == 2:
        ...
        packname = f"jar_{user.id}_{pack}"
        ...
        if (
            ...
            async with jarvis_bot.conversation("@Stickers") as conv:
                try:
                    await conv.send_message("/addsticker")
                except YouBlockedUserError:
                    LOGS.info("Unblocking @Stickers for kang...")
                    await jarvis_bot(functions.contacts.UnblockRequest("stickers"))
                    await conv.send_message("/addsticker")
                ...
                await conv.get_response()
                await jarvis_bot.send_read_acknowledge(conv.chat_id)
        else:
...
async def jardround(event):
    ...
    Image.fromarray(npImage).save("jar.webp")
...
async def jardestroy(event):
    jar = await event.get_reply_message()
    if not (jar and jar.media and "animated" in mediainfo(jar.media)):
        return await event.eor(get_string("sts_2"))
    await event.client.download_media(jar, "jarvis.tgs")
    xx = await event.eor(get_string("com_1"))
    await bash("lottie_convert.py jarvis.tgs json.json")
    ...
    file = await con.animated_sticker("json.json", "jarvis.tgs")
    if file:
        await event.client.send_file(
            event.chat_id,
            file="jarvis.tgs",
...
async def jatiny(event):
    ...
    if ik.endswith(".tgs"):
        ...
        open("json.json", "w").write(jsn)
        await con.animated_sticker("json.json", "jar.tgs")
        file = "jar.tgs"
        os.remove("json.json")
