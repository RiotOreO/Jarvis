...
@jarvis_cmd(pattern="kickme$", fullsudo=True)
async def leave(jar):
    await jar.eor(f"`{jar.client.me.first_name} has left this group, bye!!.`")
    await jar.client(LeaveChannelRequest(jar.chat_id))
...
@jarvis_cmd(pattern="paste( (.*)|$)", manager=True, allow_all=True)
async def _(event):
    try:
...
async def _(jar):
    ...
    to_add_users = jar.pattern_match.group(1).strip()
    if not jar.is_channel and jar.is_group:
        for user_id in to_add_users.split(" "):
            try:
                await jar.client(
                    AddChatUserRequest(
                        chat_id=jar.chat_id,
                        user_id=await jar.client.parse_id(user_id),
                        fwd_limit=1000000,
                    ),
                )
                await xx.edit(f"Successfully invited `{user_id}` to `{jar.chat_id}`")
        ...
        for user_id in to_add_users.split(" "):
            try:
                await jar.client(
                    InviteToChannelRequest(
                        channel=jar.chat_id,
                        users=[await jar.client.parse_id(user_id)],
                    ),
                )
                await xx.edit(f"Successfully invited `{user_id}` to `{jar.chat_id}`")
            except UserBotError:
...
async def abs_rmbg(event):
    ...
    if zz.mode != "RGB":
        zz.convert("RGB")
    wbn = check_filename("jar-rmbg.webp")
    zz.save(wbn, "webp")
...
async def telegraphcmd(event):
    xx = await event.eor(get_string("com_1"))
    match = event.pattern_match.group(1).strip() or "Jarvis"
    reply = await event.get_reply_message()
    if not reply:
...
@jarvis_cmd(pattern="suggest( (.*)|$)", manager=True)
