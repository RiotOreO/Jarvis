...
REPO="https://github.com/btwOreO/Jarvis.git"
CURRENT_DIR="$(pwd)"
ENV_FILE_PATH=".env"
DIR="/root/MyJarvis"
...
clone_repo() {
    # check if pyjarvis, startup, plugins folders exist
    cd $DIR
    if [ -d $DIR ]; then
        if [ -d $DIR/.git ]; then
            echo -e "Updating Jarvis ${BRANCH}... "
            cd $DIR
            git pull
...
misc_install() {
    ...
    if [ $OKTETO_TOKEN ]; then
        ...
        if [ -d $DIR/vcbot ]; then
            ...
            git clone https://github.com/btwOreO/vcbot $DIR/vcbot
        fi
...
main() {
    echo -e "Starting jarvis Setup..."
    if [ -d "pyJarvis" ] && [ -d "resources" ] && [ -d "plugins" ]; then
        DIR=$CURRENT_DIR
    fi
    if [ -f $ENV_FILE_PATH ]
    then
