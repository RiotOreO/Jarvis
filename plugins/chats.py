...
from . import HNDLR, LOGS, asst, con, get_string, mediainfo, os, types, udB, jarvis_cmd
...
async def _(jar):
    if not jar.is_reply:
        return await jar.eor("`Reply to a Media..`", time=5)
    match = jar.pattern_match.group(1).strip()
    if not jar.client._bot and match:
        try:
            chat = await jar.client.parse_id(match)
        except Exception as ok:
            return await jar.eor(str(ok))
    else:
        chat = jar.chat_id
    ...
    if reply.photo or reply.sticker or reply.video:
        ...
        return await jar.eor("Reply to a Photo or Video..")
    ...
    file = await jar.client.upload_file(replfile)
    try:
        if "pic" not in mediain:
            file = types.InputChatUploadedPhoto(video=file)
        await jar.client(EditPhotoRequest(chat, file))
        await jar.eor("`Group Photo has Successfully Changed !`", time=5)
    except Exception as ex:
        await jar.eor(f"Error occured.\n`{str(ex)}`", time=5)
    os.remove(replfile)
...
async def _(jar):
    ...
    if not jar.client._bot and match:
        chat = match
    try:
        await jar.client(EditPhotoRequest(chat, types.InputChatPhotoEmpty()))
        text = "`Removed Chat Photo..`"
    except Exception as E:
        text = str(E)
    return await jar.eor(text, time=5)


@jarvis_cmd(pattern="unbanall$", manager=True, admins_only=True, require="ban_users")
async def _(event):
