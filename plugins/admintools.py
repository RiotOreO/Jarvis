...
async def prmte(jar):
    xx = await jar.eor(get_string("com_1"))
    user, rank = await get_uinfo(jar)
    rank = rank or "Admin"
    FullRight = False
    if not user:
    ...
    try:
        if FullRight:
            await jar.client(
                EditAdminRequest(jar.chat_id, user.id, jar.chat.admin_rights, rank)
            )
        else:
            await jar.client.edit_admin(
                jar.chat_id,
                user.id,
                invite_users=True,
                ban_users=True,
        ...
        await eod(
            xx, get_string("pro_2").format(inline_mention(user), jar.chat.title, rank)
        )
...
async def dmote(jar):
    ...
    try:
        ...
        await eod(xx, get_string("de_2").format(inline_mention(user), jar.chat.title))
    except Exception as ex:
        return await xx.edit(f"`{ex}`")
...
async def bban(jar):
    something = await get_uinfo(jar)
    if not something:
        return
    user, reason = something
    if not user:
        return await eod(jar, get_string("ban_1"))
    if user.id in DEVLIST:
        return await eod(jar, get_string("ban_2"))
    try:
        await jar.client.edit_permissions(jar.chat_id, user.id, view_messages=False)
    except UserIdInvalidError:
        return await eod(jar, get_string("adm_1"))
    except BadRequestError:
        return await eod(jar, get_string("ban_3"))
    senderme = inline_mention(await jar.get_sender())
    userme = inline_mention(user)
    text = get_string("ban_4").format(userme, senderme, jar.chat.title)
    if reason:
...
async def uunban(jar):
    xx = await jar.eor(get_string("com_1"))
    if jar.text[1:].startswith("unbanall"):
        return
    something = await get_uinfo(jar)
    if not something:
        return
    ...
    try:
        await jar.client.edit_permissions(jar.chat_id, user.id, view_messages=True)
    except UserIdInvalidError:
        return await eod(jar, get_string("adm_1"))
    except BadRequestError:
        return await xx.edit(get_string("adm_2"))
    sender = inline_mention(await jar.get_sender())
    text = get_string("unban_3").format(inline_mention(user), sender, jar.chat.title)
    if reason:
        text += get_string("ban_5").format(reason)
    await xx.edit(text)
...
async def kck(jar):
    if "kickme" in jar.text:
        return
    if jar.is_private:
        return await jar.eor("`Use this in Group/Channel.`", time=5)
    ml = jar.text.split(" ", maxsplit=1)[0]
    xx = await jar.eor(get_string("com_1"))
    something = await get_uinfo(jar)
    if not something:
        return
    ...
    try:
        await jar.client.kick_participant(jar.chat_id, user.id)
    ...
    text = get_string("kick_4").format(
        inline_mention(user), inline_mention(await jar.get_sender()), jar.chat.title
    )
    if reason:
        text += get_string("ban_5").format(reason)
    await xx.edit(text)
...
@jarvis_cmd(pattern="pin$", manager=True, require="pin_messages", fullsudo=True)
async def pin(msg):
    if not msg.is_reply:
...
async def unp(jar):
    xx = await jar.eor(get_string("com_1"))
    ch = (jar.pattern_match.group(1).strip()).strip()
    msg = None
    if jar.is_reply:
        msg = jar.reply_to_msg_id
    elif ch != "all":
        return await xx.edit(get_string("unpin_1").format(HNDLR))
    try:
        await jar.client.unpin_message(jar.chat_id, msg)
    except BadRequestError:
        return await xx.edit(get_string("adm_2"))
    except Exception as e:
...
async def pin_message(jar):
    match = jar.pattern_match.group(1).strip()
    if not jar.is_reply:
        return await jar.eor("`Reply to message..`", time=6)
    if not match:
        return await jar.eor("`Please provide time..`", time=8)
    msg = await jar.eor(get_string("com_1"))
    msg_id = jar.reply_to_msg_id
    try:
        time = ban_time(match)
        await jar.client.pin_message(jar.chat_id, msg_id)
        await msg.eor(f"`pinned for time` `{time}`", time=8)
    except Exception as er:
        return await msg.edit(str(er))
    await asyncio.sleep(time)
    try:
        await jar.client.unpin_message(jar.chat_id, msg_id)
    except Exception as er:
        LOGS.exception(er)


@jarvis_cmd(pattern="purge( (.*)|$)", manager=True, require="delete_messages")
...
@jarvis_cmd(pattern="pinned", manager=True, groups_only=True)
...
async def autodelte(jar):
    match = jar.pattern_match.group(1).strip()
    if not match or match not in ["24h", "7d", "1m", "off"]:
        return await jar.eor("`Please Use in Proper Format..`", time=5)
    if match == "24h":
        tt = 3600 * 24
    elif match == "7d":
        tt = 3600 * 24 * 7
        ...
        tt = 0
    try:
        await jar.client(SetHistoryTTLRequest(jar.chat_id, period=tt))
    except ChatNotModifiedError:
        return await jar.eor(
            f"Auto Delete Setting is Already same to `{match}`", time=5
        )
    await jar.eor(f"Auto Delete Status Changed to `{match}` !")
