...
if run_as_module:
    ...
    async def safeinstall(event):
        ...
        if plug in HELP:
            ...
            for i in HELP[plug]:
                output += i
            output += "\n© @MyJarvis"
            await eod(ok, f"✓ `Jarvis - Installed`: `{plug}` ✓\n\n{output}")
    ...
    async def heroku_logs(event):
        ...
        ok = app.get_log()
        with open("jarvis-heroku.log", "w") as log:
            log.write(ok)
        await event.client.send_file(
            event.chat_id,
            file="jarvis-heroku.log",
            thumb=JARConfig.thumb,
            caption="**Jarvis Heroku Logs.**",
        )

        os.remove("jarvis-heroku.log")
        await xx.delete()

    async def def_logs(jar, file):
        await jar.respond(
            "**Jarvis Logs.**",
            file=file,
            thumb=JARConfig.thumb,
    ...
    def gen_chlog(repo, diff):
        ...
        ch_log = tldr_log = ""
        ch = f"<b>Jarvis {jarvis_version} updates for <a href={UPSTREAM_REPO_URL}/tree/{ac_br}>[{ac_br}]</a>:</b>"
        ch_tl = f"Jarvis {jarvis_version} updates for {ac_br}:"
        d_form = "%d/%m/%y || %H:%M"
...
async def restart(jar=None):
    if Var.HEROKU_APP_NAME and Var.HEROKU_API:
        try:
            ...
            if jar:
                await jar.edit("`Restarting your app, please wait for a minute!`")
            app.restart()
...
async def shutdown(jar):
    from .. import HOSTED_ON, LOGS

    jar = await eor(jar, "Shutting Down")
    if HOSTED_ON == "heroku":
        if not (Var.HEROKU_APP_NAME and Var.HEROKU_API):
            return await jar.edit("Please Fill `HEROKU_APP_NAME` and `HEROKU_API`")
        dynotype = os.getenv("DYNO").split(".")[0]
        try:
            Heroku = heroku3.from_key(Var.HEROKU_API)
            app = Heroku.apps()[Var.HEROKU_APP_NAME]
            await jar.edit("`Shutting Down your app, please wait for a minute!`")
            app.process_formation()[dynotype].scale(0)
        except BaseException as e:
            LOGS.exception(e)
            return await jar.edit(
                "`HEROKU_API` and `HEROKU_APP_NAME` is wrong! Kindly re-check in config vars."
            )
    else:
        sys.exit()
