...
async def autoupdate_local_database():
    from .. import Var, asst, udB, jarvis_bot

    global db_url
    db_url = (
        udB.get_key("TGDB_URL") or Var.TGDB_URL or jarvis_bot._cache.get("TGDB_URL")
    )
    if db_url:
        _split = db_url.split("/")
        _channel = _split[-2]
...
async def startup_stuff():
    ...
    if CT:
        ...
        JARConfig.thumb = path
        try:
            await download_file(CT, path)
        except Exception as er:
            LOGS.exception(er)
    elif CT is False:
        JARConfig.thumb = None
    ...
    if TZ and timezone:
        try:
            ...
            LOGS.critical(
                "Incorrect Timezone ,\nCheck Available Timezone From Here https://graph.org/Jarvis-10-28-4 \nSo Time is Default UTC"
            )
            os.environ["TZ"] = "UTC"
            time.tzset()
...
async def autobot():
    from .. import udB, jarvis_bot

    if udB.get_key("BOT_TOKEN"):
        return
    await jarvis_bot.start()
    ...
    if who.username:
        ...
        username = "jarvis_" + (str(who.id))[5:] + "_bot"
    bf = "@BotFather"
    await jarvis_bot(UnblockRequest(bf))
    await jarvis_bot.send_message(bf, "/cancel")
    await asyncio.sleep(1)
    await jarvis_bot.send_message(bf, "/newbot")
    await asyncio.sleep(1)
    isdone = (await jarvis_bot.get_messages(bf, limit=1))[0].text
    if isdone.startswith("That I cannot do.") or "20 bots" in isdone:
        ...
        sys.exit(1)
    await jarvis_bot.send_message(bf, name)
    await asyncio.sleep(1)
    isdone = (await jarvis_bot.get_messages(bf, limit=1))[0].text
    if not isdone.startswith("Good."):
        await jarvis_bot.send_message(bf, "My Assistant Bot")
        await asyncio.sleep(1)
        isdone = (await jarvis_bot.get_messages(bf, limit=1))[0].text
        if not isdone.startswith("Good."):
            LOGS.critical(
            ...
            sys.exit(1)
    await jarvis_bot.send_message(bf, username)
    await asyncio.sleep(1)
    isdone = (await jarvis_bot.get_messages(bf, limit=1))[0].text
    await jarvis_bot.send_read_acknowledge("botfather")
    if isdone.startswith("Sorry,"):
        ran = randint(1, 100)
        username = "jarvis_" + (str(who.id))[6:] + str(ran) + "_bot"
        await jarvis_bot.send_message(bf, username)
        await asyncio.sleep(1)
        isdone = (await jarvis_bot.get_messages(bf, limit=1))[0].text
    if isdone.startswith("Done!"):
        ...
        await enable_inline(jarvis_bot, username)
        LOGS.info(
...
async def autopilot():
    from .. import asst, udB, jarvis_bot

    channel = udB.get_key("LOG_CHANNEL")
    new_channel = None
    if channel:
        try:
            chat = await jarvis_bot.get_entity(channel)
    ...
    if not channel:
        ...
        async def _save(exc):
            udB._cache["LOG_CHANNEL"] = jarvis_bot.me.id
            await asst.send_message(
                jarvis_bot.me.id, f"Failed to Create Log Channel due to {exc}.."
            )
        ...
        try:
            r = await jarvis_bot(
                CreateChannelRequest(
                    title="My Jarvis Logs",
                    about="My Jarvis Log Group\n\n Join @MyJarvis",
                    megagroup=True,
                ),
            )
    ...
    assistant = True
    try:
        await jarvis_bot.get_permissions(int(channel), asst.me.username)
    except UserNotParticipantError:
        try:
            await jarvis_bot(InviteToChannelRequest(int(channel), [asst.me.username]))
        except BaseException as er:
    ...
    if isinstance(chat.photo, ChatPhotoEmpty):
        photo, _ = await download_file(
            "https://raw.githubusercontent.com/btwOreO/JarvisMedia/refs/heads/main/img6.jpg", "channelphoto.jpg"
        )
        ll = await jarvis_bot.upload_file(photo)
        try:
            await jarvis_bot(
...
async def customize():
    ...
    try:
        ...
        await asyncio.sleep(1)
        await jarvis_bot.send_message("botfather", "/cancel")
        await asyncio.sleep(1)
        await jarvis_bot.send_message("botfather", "/setuserpic")
        await asyncio.sleep(1)
        isdone = (await jarvis_bot.get_messages("botfather", limit=1))[0].text
        if isdone.startswith("Invalid bot"):
            LOGS.info("Error while trying to customise assistant, skipping...")
            return
        await jarvis_bot.send_message("botfather", UL)
        await asyncio.sleep(1)
        await jarvis_bot.send_file("botfather", file)
        await asyncio.sleep(2)
        await jarvis_bot.send_message("botfather", "/setabouttext")
        await asyncio.sleep(1)
        await jarvis_bot.send_message("botfather", UL)
        await asyncio.sleep(1)
        await jarvis_bot.send_message(
            "botfather", f"✨ Hello ✨!! I'm Assistant Bot of {sir}"
        )
        await asyncio.sleep(2)
        await jarvis_bot.send_message("botfather", "/setdescription")
        await asyncio.sleep(1)
        await jarvis_bot.send_message("botfather", UL)
        await asyncio.sleep(1)
        await jarvis_bot.send_message(
            "botfather",
            f"✨ Powerful Jarvis Assistant Bot ✨\n✨ Master ~ {sir} ✨\n\n✨ Powered By ~ @MyJarvis ✨",
        )
        await asyncio.sleep(2)
...
async def plug(plugin_channels):
    ...
    for chat in plugin_channels:
        LOGS.info(f"{'•'*4} {chat}")
        try:
            async for x in jarvis_bot.iter_messages(
                ...
                if not os.path.exists(plugin):
                    ...
                    try:
                        ...
                        LOGS.info(f"Jarvis - PLUGIN_CHANNEL - ERROR - {plugin}")
                        LOGS.exception(e)
                        os.remove(plugin)
        except Exception as er:
            LOGS.exception(er)
...
async def fetch_ann():
    ...
    try:
        updts = await async_searcher(
            "https://jarvis-api.vercel.app/announcements", post=True, re_json=True
        )
        for upt in updts:
            key = list(upt.keys())[0]
            if key not in get_:
...
async def ready():
    ...
    if not udB.get_key("INIT_DEPLOY"):  # Detailed Message at Initial Deploy
        MSG = """🎇 **Thanks for Deploying Jarvis Userbot!**
• Here, are the Some Basic stuff from, where you can Know, about its Usage."""
        PHOTO = "https://raw.githubusercontent.com/btwOreO/JarvisMedia/refs/heads/main/img7.jpg"
        BTTS = Button.inline("• Click to Start •", "initft_2")
        udB.set_key("INIT_DEPLOY", "Done")
    else:
        MSG = f"**Jarvis has been deployed!**\n➖➖➖➖➖➖➖➖➖➖\n**UserMode**: {inline_mention(jarvis_bot.me)}\n**Assistant**: @{asst.me.username}\n➖➖➖➖➖➖➖➖➖➖\n**Support**: @MyJarvis\n➖➖➖➖➖➖➖➖➖➖"
        BTTS, PHOTO = None, None
        prev_spam = udB.get_key("LAST_UPDATE_LOG_SPAM")
        if prev_spam:
            try:
                await jarvis_bot.delete_messages(chat_id, int(prev_spam))
            except Exception as E:
    ...
    try:
        ...
        try:
            await (await jarvis_bot.send_message(chat_id, str(e))).delete()
            spam_sent = await asst.send_message(chat_id, MSG, file=PHOTO, buttons=BTTS)
        ...
        LOGS.info(el)
        try:
            spam_sent = await jarvis_bot.send_message(chat_id, MSG)
...
async def WasItRestart(udb):
    ...
    if not key:
        return
    from .. import asst, jarvis_bot

    try:
        data = key.split("_")
        who = asst if data[0] == "bot" else jarvis_bot
...
async def enable_inline(jarvis_bot, username):
    bf = "BotFather"
    await jarvis_bot.send_message(bf, "/setinline")
    await asyncio.sleep(1)
    await jarvis_bot.send_message(bf, f"@{username}")
    await asyncio.sleep(1)
    await jarvis_bot.send_message(bf, "Search")
    await jarvis_bot.send_read_acknowledge(bf)
