...
async def warn(e):
    ...
    if reply:
        ...
        try:
            user = e.text.split()[1]
            if user.startswith("@"):
                ok = await jarvis_bot.get_entity(user)
                user = ok.id
            else:
                user = int(user)
    ...
    if count + 1 >= number:
        if "ban" in action:
            try:
                await jarvis_bot.edit_permissions(e.chat_id, user, view_messages=False)
            except BaseException:
                return await e.eor("`Something Went Wrong.`", time=5)
        elif "kick" in action:
            try:
                await jarvis_bot.kick_participant(e.chat_id, user)
            except BaseException:
                return await e.eor("`Something Went Wrong.`", time=5)
        elif "mute" in action:
            try:
...
@jarvis_cmd(pattern="setwarn( (.*)|$)", manager=True)
async def warnset(e):
