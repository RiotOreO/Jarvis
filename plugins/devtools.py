...
@jarvis_cmd(pattern="bash", fullsudo=True, only_devs=True)
async def _(event):
    ...
    if len(OUT) > 4096:
        jrvs = err + out
        with BytesIO(str.encode(jrvs)) as out_file:
...
@jarvis_cmd(pattern="eval", fullsudo=True, only_devs=True)
async def _(event):
    try:
        cmd = event.text.split(maxsplit=1)[1]
    except IndexError:
    ...
    if any(item in cmd for item in KEEP_SAFE().All) and (
        not (event.out or event.sender_id == jarvis_bot.uid)
    ):
...
@jarvis_cmd(pattern="cpp", only_devs=True)
async def doie(e):
    ...
    open("cpp-jarvis.cpp", "w").write(match)
    m = await bash("g++ -o CppJarvis cpp-jarvis.cpp")
    o_cpp = f"• **Eval-Cpp**\n`{match}`"
    if m[1]:
        o_cpp += f"\n\n**• Error :**\n`{m[1]}`"
        if len(o_cpp) > 3000:
            os.remove("cpp-jarvis.cpp")
            if os.path.exists("CppJarvis"):
                os.remove("CppJarvis")
