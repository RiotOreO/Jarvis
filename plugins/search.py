...
async def gitsearch(event):
    ...
    jar = await async_searcher(url, re_json=True)
    try:
        uname = jar["login"]
        uid = jar["id"]
        ...
        ucomp = jar["company"]
        ublog = jar["blog"]
        ulocation = jar["location"]
        ubio = jar["bio"]
        urepos = jar["public_repos"]
        ufollowers = jar["followers"]
        ufollowing = jar["following"]
...
@jarvis_cmd(pattern="reverse$")
async def reverse(event):
    reply = await event.get_reply_message()
    if not reply:
        return await event.eor("`Reply to an Image`")
    jar = await event.eor(get_string("com_1"))
    ...
    link = alls["href"]
    text = alls.text
    await jar.edit(f"`Dimension ~ {x} : {y}`\nSauce ~ [{text}](google.com{link})")
