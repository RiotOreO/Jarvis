...
async def source(e):
    ...
    if not SourceM.contains(y):
        ...
        jarvis_bot.add_handler(autopost_func, events.NewMessage())
    else:
        await e.eor(get_string("cha_3"))


@jarvis_cmd(pattern="dsource( (.*)|$)")
async def dd(event):
...
@jarvis_cmd(pattern="listdest")
async def list_all(event):
    jarvis_bot = event.client
    x = await event.eor(get_string("com_1"))
    channels = DestiM.get()
    num = len(channels)
    if not num:
    ...
    for channel in channels:
        name = ""
        try:
            name = get_display_name(await jarvis_bot.get_entity(int(channel)))
        except BaseException:
            name = ""
        msg += f"\n=> **{name}** [`{channel}`]"
    msg += f"\nTotal {num} channels."
