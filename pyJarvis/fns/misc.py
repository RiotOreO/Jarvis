...
async def randomchannel(
    tochat, channel, range1, range2, caption=None, client=jarvis_bot
):
...
async def allcmds(event, telegraph):
    ...
    for z in LIST.keys():
        ...
        for zz in LIST[z]:
            txt += HNDLR + zz + "\n"
        txt += "\n\n"
    t = telegraph.create_page(title="Jarvis All Cmds", content=[txt])
    await eor(event, f"All Jarvis Cmds : [Click Here]({t['url']})", link_preview=False)
...
async def ReTrieveFile(input_file_name):
    ...
    async with aiohttp.ClientSession() as ses:
        async with ses.post(
            ...
            name = check_filename("jar-rmbg.png")
