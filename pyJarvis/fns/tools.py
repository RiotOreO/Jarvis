...
def check_filename(filroid):
    if os.path.exists(filroid):
        no = 1
        while True:
            jar = "{0}_{2}{1}".format(*os.path.splitext(filroid) + (no,))
...
def telegraph_client():
    ...
    gd_name = jarvis_bot.full_name
    short_name = gd_name[:30]
    profile_url = (
        f"https://t.me/{jarvis_bot.me.username}"
        if jarvis_bot.me.username
        else "https://t.me/MyJarvis"
    )
    try:
        ...
        if "SHORT_NAME_TOO_LONG" in str(er):
            TelegraphClient.create_account(
                short_name="jarvisuser", author_name=gd_name, author_url=profile_url
            )
        else:
            LOGS.exception(er)
            return
...
async def Carbon(
    ...
    **kwargs,
    ...
    if rayso:
        ...
        kwargs["title"] = kwargs.get("title", "Jarvis")
    else:
        kwargs["code"] = code
