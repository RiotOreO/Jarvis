...
def telethon_session():
    ...
    # logging in
    try:
        with TelegramClient(StringSession(), API_ID, API_HASH) as jarvis:
            print("Generating a string session for •JARVIS•")
            try:
                jarvis.send_message(
                    "me",
        ...
        print(er)
        print("If you think It as a Bug, Report to @JarvisSupportChat.\n\n")
...
def pyro_session():
    ...
    try:
        with Client(name="jarvis", api_id=API_ID, api_hash=API_HASH, in_memory=True) as pyro:
            ss = pyro.export_session_string()
            pyro.send_message(
                "me",
                f"`{ss}`\n\nAbove is your Pyrogram Session String for @MyJarvis. **DO NOT SHARE it.**",
            )
...
def main():
    ...
    try:
        type_of_ss = int(
            input(
                "\nJarvis supports both telethon as well as pyrogram sessions.\n\nWhich session do you want to generate?\n1. Telethon Session.\n2. Pyrogram Session.\n\nEnter choice:  "
            )
        )
