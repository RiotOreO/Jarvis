...
@jarvis_cmd(pattern="gpromote( (.*)|$)", fullsudo=True)
async def _(e):
...
@jarvis_cmd(pattern="gdemote( (.*)|$)", fullsudo=True)
async def _(e):
    x = e.pattern_match.group(1).strip()
    jarvis_bot = e.client
    if not x:
    ...
    if user:
        ...
        if user.isdigit():
            user = int(user)
        try:
            name = await jarvis_bot.get_entity(user)
        except BaseException:
            return await e.eor(f"`No User Found Regarding {user}`", time=5)
        ...
        rank = "Not AdMin"
        c = 0
        async for x in jarvis_bot.iter_dialogs():
            if (
                "group" in key.lower()
                and x.is_group
                or "group" not in key.lower()
...
@jarvis_cmd(pattern="ungban( (.*)|$)", fullsudo=True)
async def _(e):
...
@jarvis_cmd(pattern="gban( (.*)|$)", fullsudo=True)
async def _(e):
...
@jarvis_cmd(pattern="g(admin|)cast( (.*)|$)", fullsudo=True)
...
@jarvis_cmd(pattern="gucast( (.*)|$)", fullsudo=True)
...
@jarvis_cmd(pattern="gkick( (.*)|$)", fullsudo=True)
async def gkick(e):
...
@jarvis_cmd(pattern="gmute( (.*)|$)", fullsudo=True)
async def _(e):
...
@jarvis_cmd(pattern="ungmute( (.*)|$)", fullsudo=True)
async def _(e):
...
async def list_gengbanned(event):
    ...
    if len(gbanned_users) > 4096:
        ...
        await x.reply(
            file="gbanned.txt",
            message=f"List of users GBanned by {inline_mention(jarvis_bot.me)}",
        )
...
@jarvis_cmd(pattern="gblacklist$")
async def blacklist_(event):
    await gblacker(event, "add")


@jarvis_cmd(pattern="ungblacklist$")
