...
@jarvis_cmd(pattern="fsub( (.*)|$)", admins_only=True, groups_only=True)
async def addfor(e):
    ...
    jarvis_bot.add_handler(force_sub, events.NewMessage(incoming=True))


@jarvis_cmd(pattern="remfsub$")
async def remor(e):
...
async def fcall(e):
    match = e.pattern_match.group(1).strip()
    spli = match.split("_")
    user = await jarvis_bot.get_entity(int(spli[0]))
    cl = await jarvis_bot.get_entity(int(spli[1]))
    text = f"Hi {inline_mention(user)}, You Need to Join"
    text += f" {cl.title} in order to Chat in this Group."
    el = (
        f"https://t.me/{cl.username}"
        if cl.username
        else (await jarvis_bot(ExportChatInviteRequest(cl))).link
    )
...
async def diesoon(e):
    ...
    try:
        values = await jarvis_bot(GetParticipantRequest(int(spli[1]), int(spli[0])))
...
async def force_sub(jar):
    if not udB.get_key("FORCESUB"):
        return
    user = await jar.get_sender()
    joinchat = get_forcesetting(jar.chat_id)
    if (not joinchat) or (isinstance(user, User) and user.bot):
        return
    if CACHE.get(jar.chat_id):
        if CACHE[jar.chat_id].get(user.id):
            CACHE[jar.chat_id].update({user.id: CACHE[jar.chat_id][user.id] + 1})
        else:
            CACHE[jar.chat_id].update({user.id: 1})
    else:
        CACHE.update({jar.chat_id: {user.id: 1}})
    count = CACHE[jar.chat_id][user.id]
    if count == 11:
        CACHE[jar.chat_id][user.id] = 1
        return
    if count in range(2, 11):
        return
    try:
        await jarvis_bot.get_permissions(int(joinchat), user.id)
        return
    except UserNotParticipantError:
        pass
    if isinstance(user, Channel):
        try:
            await jarvis_bot.edit_permissions(
                jar.chat_id, user.id, view_messages=False
            )
            return
        except BaseException as er:
            LOGS.exception(er)
    try:
        await jarvis_bot.edit_permissions(jar.chat_id, user.id, send_messages=False)
    except ChatAdminRequiredError:
        return
    except Exception as e:
        await jar.delete()
        LOGS.info(e)
    res = await jarvis_bot.inline_query(asst.me.username, f"fsub {user.id}_{joinchat}")
    await res[0].click(jar.chat_id, reply_to=jar.id)
