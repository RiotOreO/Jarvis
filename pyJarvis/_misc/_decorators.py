...
from ..version import jarvis_version as jar_ver
...
def jarvis_cmd(
    pattern=None, manager=False, jarvis_bot=jarvis_bot, asst=asst, **kwargs
):
    ...
    def decor(dec):
        async def wrapp(jar):
            if not jar.out:
                if owner_only:
                    return
                if jar.sender_id not in owner_and_sudos():
                    return
                if jar.sender_id in _ignore_eval:
                    return await eod(
                        jar,
                        get_string("py_d1"),
                    )
                if fullsudo and jar.sender_id not in SUDO_M.fullsudos:
                    return await eod(jar, get_string("py_d2"), time=15)
            chat = jar.chat
            if hasattr(chat, "title"):
                if (
                    "#noub" in chat.title.lower()
                    and not (chat.admin_rights or chat.creator)
                    and not (jar.sender_id in DEVLIST)
                ):
                    return
            if jar.is_private and (groups_only or admins_only):
                return await eod(jar, get_string("py_d3"))
            elif admins_only and not (chat.admin_rights or chat.creator):
                return await eod(jar, get_string("py_d5"))
            ...
            try:
                ...
                await jarvis_bot.disconnect()
                await asyncio.sleep(fwerr.seconds + 10)
                await jarvis_bot.connect()
                await asst.send_message(
                    udB.get_key("LOG_CHANNEL"),
                ...
                return await eod(jar, "`Inline Locked In This Chat.`")
            except (ChatSendMediaForbiddenError, ChatSendStickersForbiddenError):
                return await eod(jar, get_string("py_d8"))
            except (BotMethodInvalidError, UserIsBotError):
                return await eod(jar, get_string("py_d6"))
                ...
                ftext = "**Jarvis Client Error:** `Forward this to` @JarvisSupportChat\n\n"
                ftext += "**Py-Jarvis Version:** `" + str(pyver)
                ftext += "`\n**Jarvis Version:** `" + str(jar_ver)
                ftext += "`\n**Telethon Version:** `" + str(telever)
                ftext += f"`\n**Hosted At:** `{HOSTED_ON}`\n\n"
                ftext += "--------START JARVIS CRASH LOG--------"
                ftext += "\n**Date:** `" + date
                ftext += "`\n**Group:** `" + str(jar.chat_id) + "` " + str(naam)
                ftext += "\n**Sender ID:** `" + str(jar.sender_id)
                ftext += "`\n**Replied:** `" + str(jar.is_reply)
                ftext += "`\n\n**Event Trigger:**`\n"
                ftext += str(jar.text)
                ftext += "`\n\n**Traceback info:**`\n"
                ...
                if len(ftext) > 4096:
                    with BytesIO(ftext.encode()) as file:
                        ...
                        error_log = await asst.send_file(
                            udB.get_key("LOG_CHANNEL"),
                            file,
                            caption="**Jarvis Client Error:** `Forward this to` @JarvisSupportChat\n\n",
                        )
                else:
        ...
        if manager and MANAGER:
            ...
            async def manager_cmd(jar):
                if not allow_all and not (await admin_check(jar, require=require)):
                    return
                if not allow_pm and jar.is_private:
                    return
                try:
                    await dec(jar)
                except Exception as er:
                    if chat := udB.get_key("MANAGER_LOG"):
                        text = f"**#MANAGER_LOG\n\nChat:** `{get_display_name(jar.chat)}` `{jar.chat_id}`"
                        text += f"\n**Replied :** `{jar.is_reply}`\n**Command :** {jar.text}\n\n**Error :** `{er}`"
                        try:
                            ...
                            LOGS.exception(er)
                    LOGS.info(f"• MANAGER [{jar.chat_id}]:")
                    LOGS.exception(er)
